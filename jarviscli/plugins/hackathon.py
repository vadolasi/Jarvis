...
class Hackathon:
    ...
    def get_csrf_token(self):
        ...
        csrf_token = [
            line
            for line in csrf_request.text.split("\n")
            if "var CSRF_COOKIE =" in line
        ][0]
    ...
    def format(self, data, jarvis):
        ...
        if upcoming is not None:
            ...
            for i, hackathon in enumerate(all_hackathons, 1):
                challenge_type = (
                    hackathon.find("div", {"class": "challenge-type"})
                    .text.replace("\n", " ")
                    .strip()
                )
                challenge_name = (
                    hackathon.find("div", {"class": "challenge-name"})
                    .text.replace("\n", " ")
                    .strip()
                )
                date_time = (
                ...
                jarvis.say(
                    "[{}] {}\n{}\n{}\n\n".format(
                        str(i), challenge_name, challenge_type, date_time
                    )
                )
        else:
