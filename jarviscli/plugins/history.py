...
class history:
    ...
    * history <event> <day> <month>
    ...
    def _print_help(self, jarvis):
        ...
        jarvis.say(
            "    <keyword> - Program uses special keywords to easily identify your query. keywords:",
            Fore.CYAN,
        )
        jarvis.say(
            "                * 'yesterday' - results in getting fact that happened day before today",
            Fore.CYAN,
        )
        jarvis.say(
            "                * 'today' - results in getting fact that happened on this day",
            Fore.CYAN,
        )
        jarvis.say(
            "                * 'tomorrow' - results in getting fact that happened day after today",
            Fore.CYAN,
        )
        jarvis.say(
        ...
        jarvis.say(
            "All of the arguments are optional. Not specifying results in randomization.",
            Fore.CYAN,
        )
        jarvis.say("Example: ", Fore.CYAN)
        jarvis.say(
            "         'history 25 march birth' - birth of a random person on 25th of March",
            Fore.CYAN,
        )
        jarvis.say(
            "         'history 10' - random event that happened on random month but day is 10",
            Fore.CYAN,
        )
        jarvis.say(
            "         'history today' - random type of event that happened on the present day",
            Fore.CYAN,
        )
        jarvis.say(
            "         'history tomorrow events' - event that occured on the day after today",
            Fore.CYAN,
        )
    ...
    def _parse_arguments(self, args):
        ...
        for arg in split_args:
            if arg.isdigit():
                ...
                if not __validate(
                    self.KW.KEYWORD, arg, cfg, [self.KW.DAY, self.KW.MONTH]
                ):
                    return cfg
            else:
    ...
    def _get_data(self, jarvis, query, api_cfg):
        ...
        try:
            ...
            jarvis.say(
                "Please check that arguments are correct and day of month is valid!",
                Fore.RED,
            )
            jarvis.say(
    ...
    def _print_result(self, jarvis, result):
        ...
        for i in range(len(result["links"])):
            jarvis.say(
                "    {}). {}".format(i + 1, result["links"][i]["link"]), Fore.BLUE
            )
