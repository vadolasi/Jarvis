...
def game_end():
    ...
    if wpm >= 20:
        with open(
            os.path.join(FILE_PATH, "../data/typing_test_data.csv"), mode="r"
        ) as f:
            data = list(csv.reader(f))
            for i in range(len(data)):
                if int(data[i][0]) > int(wpm) and i != 0:
                    print(
                        "You are better than {}% of people!".format(
                            float(data[i - 1][1]) * 100
                        )
                    )
                    break
...
def print_screen():
    ...
    string += "\nScore - {}\t Time - {}\t Words -  \
        {}\n\n".format(
        info["score"], format_time(info["time_left"]), info["words"]
    )
