...
class News:
    ...
    def __call__(self, jarvis, s):
        if s == "updatekey":
            key = jarvis.input("Please enter your NEWS API key (q or Enter go back): ")
    ...
    def add_source(self, jarvis, news_source):
        ...
        if news_source not in sources:
            ...
            jarvis.say(
                news_source + " has been successfully been added to your sources!",
                Fore.GREEN,
            )
        else:
            jarvis.say(
                news_source + " was already included in your sources!", Fore.GREEN
            )
    ...
    def remove_source(self, jarvis):
        ...
        for index in index_list:
            if str(index) in dic:
                ...
                jarvis.update_data("news-sources", sources)
                jarvis.say(
                    source + " has been successfully removed from your news channel!",
                    Fore.GREEN,
                )
            else:
    ...
    def _get(self, jarvis, url):
        ...
        if response.status_code == requests.codes.ok:
            ...
            if response.status_code == 401:
                ...
                jarvis.say(
                    "An error occured: Error code: " + response.raise_for_status(),
                    Fore.RED,
                )
            return None
