...
class basketball:
    ...
    def get_api_key(self, jarvis):
        ...
        if m.get_data("API_KEY") is None:
            user_api_key = jarvis.input(
                "Enter Api-BasketBall.com API_KEY: ", Fore.GREEN
            )
    ...
    def update_api_key(
        ...
        user_api_key = jarvis.input(
            "Enter New Api-BasketBall.com API_KEY: ", Fore.GREEN
        )
        m = Memory("basketball.json")
    ...
    def search_request(self, jarvis, query, search_name):
        ...
        while len(value.strip()) < 3:
            print()
            print(
                "The Search {} must be at least 3 characters in length",
                search_name.lower(),
            )
            value = jarvis.input("Enter {} Name: ".format(search_name), Fore.GREEN)
        ...
        if response is None:
            jarvis.spinner_stop(
                "Error While Searching {} - Try Again Later.".format(search_name),
                Fore.YELLOW,
            )
            return
        return response
