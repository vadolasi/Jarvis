    ...
    if s != "":
        ...
        if des and srcs:
            if currentPos < 2:
                ...
                tex = " ".join(
                    words[: currentPos - 1]
                )  # Discards extra words at the end of the sentence
...
def default(jarvis):
    ...
    while (
        (srcs not in LANGUAGES)
        and (srcs not in SPECIAL_CASES)
        and (srcs not in LANGCODES)
    ):
    ...
    while (
        (des not in LANGUAGES) and (des not in SPECIAL_CASES) and (des not in LANGCODES)
    ):
